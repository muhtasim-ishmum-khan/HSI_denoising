...
def extract_patches(
    ...
    # def extract_patches_with_categories(
    #     x,
    #     attn_map,
    #     k_top=10,
    #     k_mid=10,
    ...
    # ):
    #     """
    #     Enhanced version of extract_patches that also returns category labels.
    #     """
    #     B, C, H, W = x.shape
    #     patch_list = []
    #     coords = []
    #     metadata = []
    #     categories = []  # Track patch categories
    ...
    #         # Process each category separately to maintain explicit labeling
    #         all_indices_and_categories = [
    #             (top_indices, 'top'),
    #             (mid_indices, 'mid'),
    #             (rand_indices, 'random')
    #         ]
    
    #         for indices, category in all_indices_and_categories:
    #             for idx in indices:
    #                 y, x_idx = idx // W, idx % W
    #                 y1 = max(0, y - (patch_size // 2))
    #                 x1 = max(0, x_idx - (patch_size // 2))
    ...
    #     return patch_list, coords, metadata, categories

    # def save_patches_to_mat(patches, coords, metadata, categories, output_path, 
    #                    original_filename, k_top=10, k_mid=10, k_rand=10):
    #     """
    #     Save extracted patches to a single .mat file with organized structure.
        
    #     Args:
    #         patches: List of torch tensors, each of shape (C, patch_size, patch_size)
    #         coords: List of tuples (y, x) indicating patch locations
    #         metadata: List of tuples (file_name, y, x, band_idx)
    #         categories: List of strings ('top', 'mid', 'random')
    #         output_path: Path where the .mat file will be saved
    #         original_filename: Name of the original HSI file
    #     """
        
    #     # Convert patches to numpy array
    #     patches_array = torch.stack(patches).numpy()
    #     num_patches, C, H, W = patches_array.shape
    ...
    #     mat_data = {
    #         # Main patch data: (num_patches, channels, height, width)
    #         'patches': patches_array,
            
    #         # Patch categories and information
    #         'patch_categories': categories,
    #         'num_patches': num_patches,
    #         'patch_size': H,
    #         'num_channels': C,
    #         'num_top_patches': k_top,
    #         'num_mid_patches': k_mid,
    #         'num_random_patches': k_rand,
            
    #         # Location information
    #         'patch_locations_y': np.array(y_coords),
    #         'patch_locations_x': np.array(x_coords),
            
    #         # Metadata
    #         'original_filename': original_filename,
    #         'extraction_method': 'RestormerHSI_attention_guided',
    #         'patch_info': 'Patches extracted using attention-based method with categories'
    #     }
        
    #     # Save to .mat file
    #     sio.savemat(output_path, mat_data, oned_as='column')
    #     print(f" Saved {num_patches} patches to: {output_path}")
    #     print(f"   Categories: {categories.count('top')} top, {categories.count('mid')} mid, {categories.count('random')} random")
        
    #     return mat_data
    
    # def load_patches_from_mat(mat_file_path):
    #     """
    #     Load patches from a saved .mat file.
        
    #     Returns:
    #         patches: numpy array of shape (num_patches, C, H, W)
    #         metadata: dictionary with patch information
    #     """
    ...
    #     metadata = {
    #         'num_patches': int(data['num_patches'][0, 0]),
    #         'patch_size': int(data['patch_size'][0, 0]),
    #         'num_channels': int(data['num_channels'][0, 0]),
    #         'patch_locations_y': data['patch_locations_y'].flatten(),
    #         'patch_locations_x': data['patch_locations_x'].flatten(),
    #         'original_filename': str(data['original_filename'][0])
    #     }
        
    #     # Load categories if available
    #     if 'patch_categories' in data:
    #         metadata['patch_categories'] = [str(cat[0]) for cat in data['patch_categories']]
        
    #     print(f" Loaded {metadata['num_patches']} patches from: {os.path.basename(mat_file_path)}")
    #     return patches, metadata
####################################################################################################################################
